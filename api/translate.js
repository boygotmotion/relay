...
function toGoogleLang(l) {
    const dict = { "jp": "ja", "zh": "zh-CN", "ara": "ar", "kor": "ko", "ko": "ko", "fra": "fr", "spa": "es", "th": "th", "it": "it" };
...
async function translateWithGoogle(txt, f, t) {
    try {
        ...
        if (r.data && r.data[0]) {
            return r.data[0].map(s => s[0]).join("").trim();
        }
        return null;
    } catch (e) { return null; }
}
...
app.post("/api/trans/sdk/picture", upload.single("image"), async (req, res) => {
    // Note: Vercel logs appear in the Vercel Dashboard, not a local console.
    ...
    try {
        ...
        // OCR - Tesseract.js loads workers on demand
        ...
        for (let i = 0; i < fragments.length; i++) {
            ...
            // Paint white box (Serverless rendering)
...
// For Vercel check
app.get("/", (req, res) => res.send("PixPin Relay Active"));
